...
//!
//! Construction: each icon is a `2*TILE_SIZE` square canvas split into three
//! regions (a diamond top face and two parallelogram side faces) via
//! *inverse* pixel-mapping - for every destination pixel, work out which
//! face (if any) it belongs to and sample the corresponding source pixel,
//! rather than forward-mapping source pixels (which can leave gaps when
//! stretching a smaller texture over a larger area). See [`bake_cube_icon`]
//! for the exact geometry.

use std::collections::HashMap;

use crate::atlas::{AtlasData, ATLAS_PX};
use crate::blocks::{BlockId, BlockRegistry, ItemModel};
use crate::config::{ATLAS_TILES, TILE_SIZE};

/// Each icon's canvas is a square this many pixels on a side.
pub const ICON_SIZE: usize = 2 * TILE_SIZE;
/// Icon atlas dimensions - a fixed `ATLAS_TILES x ATLAS_TILES` grid of
/// `ICON_SIZE` cells, mirroring `atlas::ATLAS_PX`'s layout (mostly-empty is
/// fine; this only ever holds as many icons as there are `item_model:
/// "default"` blocks, well under the 256-cell capacity).
pub const ICON_ATLAS_PX: usize = ATLAS_TILES * ICON_SIZE;
...
/// Bakes one isometric cube icon from a top-face and a side-face tile (each
/// `TILE_SIZE*TILE_SIZE*4` RGBA bytes, row-major, as returned by
/// `extract_tile`). Returns `ICON_SIZE*ICON_SIZE*4` RGBA bytes, row-major,
...
pub fn bake_cube_icon(top: &[u8], side: &[u8]) -> Vec<u8> {
...
/// buffer into its own contiguous RGBA buffer.
fn extract_tile(atlas_pixels: &[u8], tile: u16) -> Vec<u8> {
...
/// `atlas::build_atlas`'s layout exactly, just with bigger cells. Blocks
/// using `Face`/`Custom` item models are skipped entirely (nothing to bake;
/// they render straight from the main atlas instead, see `ui::block_icon`).
pub fn build_icon_atlas(registry: &BlockRegistry, atlas: &AtlasData) -> IconAtlasData {
    ...
    for (id, def) in registry.defs.iter().enumerate().skip(1) {
        ...
        let icon = bake_cube_icon(&extract_tile(&atlas.pixels, top_tile), &extract_tile(&atlas.pixels, side_tile));
...
mod tests {
    ...
    fn solid_tile(c: [u8; 4]) -> Vec<u8> {
        let mut t = vec![0u8; TILE_SIZE * TILE_SIZE * 4];
    ...
    fn px_at(icon: &[u8], x: usize, y: usize) -> [u8; 4] {
    ...
    fn icon_has_expected_dimensions_and_is_deterministic() {
        ...
        let a = bake_cube_icon(&top, &side);
        let b = bake_cube_icon(&top, &side);
        assert_eq!(a.len(), ICON_SIZE * ICON_SIZE * 4);
        assert_eq!(a, b);
    }
    ...
    fn icon_corners_are_transparent_outside_the_cube_silhouette() {
        ...
        let icon = bake_cube_icon(&opaque, &opaque);
        let n = ICON_SIZE - 1;
        assert_eq!(px_at(&icon, 0, 0)[3], 0);
        assert_eq!(px_at(&icon, n, 0)[3], 0);
        assert_eq!(px_at(&icon, 0, n)[3], 0);
        assert_eq!(px_at(&icon, n, n)[3], 0);
    }
    ...
    fn icon_shows_all_three_faces_with_distinct_shading() {
        ...
        let icon = bake_cube_icon(&top, &side);

        // Widest row of the top diamond, dead center.
        let top_px = px_at(&icon, ICON_SIZE / 2, TILE_SIZE / 2);
        ...
        let left_px = px_at(&icon, TILE_SIZE / 2, ICON_SIZE - TILE_SIZE / 2 - 2);
        let right_px = px_at(&icon, ICON_SIZE - TILE_SIZE / 2, ICON_SIZE - TILE_SIZE / 2 - 2);
    ...
    fn build_icon_atlas_bakes_default_blocks_and_skips_face_ones() {
        ...
        assert_eq!(icons.pixels.len(), ICON_ATLAS_PX * ICON_ATLAS_PX * 4);
    }
}
